...
class RMAXAgent:
    ...
    def __init__(self, mdp, m):
        ...
        self.Q = np.ones((self.mdp.S,self.mdp.A)) * self.mdp.rmax
    ...
    def run(self):
        ...
        _, self.Q = utils.approx_q_improvement(self.mdp, self.P_hat, self.R_hat)
        self.Q[self.n_sa < self.m] = self.mdp.rmax
    ...
    def update_counts(self, trajectory):
        ...
        if self.r.shape == 3:
            self.R_hat = self.r / np.where(self.n_sas > 0, self.n_sas, 1)[:,:,np.newaxis]
        else:
            self.R_hat = self.r / np.where(self.n_sa > 0, self.n_sa, 1)[:,:,np.newaxis]
...
class data:
    ...
    # Generate trials
    @staticmethod
    def run_sim(agent, fname, ename, num_traj=50, num_reps=5):
        
        nsas = np.array([[agent.run() for i in range(num_traj)] for j in range(num_reps)])

        Path(f'{path}data/{agent.name}/{fname}/{ename}/').mkdir(parents=True,exist_ok=True)

        np.save(f'{path}data/{agent.name}/{fname}/{ename}/trial.npy', nsas)

    def plot_perf(agent, fname, ename):
        nsas = np.load(f'{path}data/{agent.name}/{fname}/{ename}/trial.npy')
...
P = np.load(f'{path}mdp_data/configs/{env_type}/{env}_P.npy')
R = np.load(f'{path}mdp_data/configs/{env_type}/{env}_R.npy')
init_state = np.load(f'{path}mdp_data/configs/{env_type}/{env}_init.npy')
goal_states = np.load(f'{path}mdp_data/configs/{env_type}/{env}_goal.npy')
loaded_mdp= mdp(P, R, init_state, goal_states)

# Update the reward if a grid mdp
if grid:
    grid_mdp.change_init_reward(loaded_mdp, goal_reward, obstacle_reward, movement_reward)

print('building baseline')
# Generate baseline policy
pi_b = build.gen_baseline(loaded_mdp, baseline)

print('gen trials')
# Collect data and plot
plot.gen_trials(path, env_type, env, method, loaded_mdp, pi_b, baseline, d_size, n_wedge, N, j_max, reps, regret)
plot.plot_perf(path, env_type, env, method, loaded_mdp, pi_b, baseline, d_size, n_wedge, N, j_max, reps, regret)
plot.plot_counts_graph(path, env_type, env, method, loaded_mdp, pi_b, baseline, d_size, n_wedge, N, j_max, reps, regret)
plot.plot_uncertainty_graph(path, env_type, env, method, loaded_mdp, pi_b, baseline, d_size, n_wedge, N, j_max, reps, regret)
plot.plot_certainty_count(path, env_type, env, method, loaded_mdp, pi_b, baseline, d_size, n_wedge, N, j_max, reps, regret)
